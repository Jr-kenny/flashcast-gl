...
const marketAddress = process.env.PREDICTION_MARKET_CONTRACT_ADDRESS || "";
...
// Poke autonomous resolution for any market whose betting window has elapsed.
// The web+LLM read and the Equivalence Principle run inside the contract's
// resolve() call. resolve() is permissionless, so this is a liveness helper, not
// a trust assumption: anyone can call it.
async function syncResolve() {
  ...
  for (const id of ids || []) {
    let m;
    try {
      m = await glClient.readContract({
        address: marketAddress, functionName: "get_market", args: [id], jsonSafeReturn: true,
      });
    } catch { continue; }
    ...
    try {
      ...
      // "not determinable yet" and "not closed yet" are expected, transient skips.
      if (!/not determinable|not closed/i.test(msg)) console.warn(`[resolve] ${id}: ${msg}`);
    }
  }
}
